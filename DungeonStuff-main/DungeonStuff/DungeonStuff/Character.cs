...
namespace DungeonStuff
{
    class Character
    {
        ...
        public static void AlliesOutput(Character[] characters)
        {
            ...
            {
                Console.SetCursorPosition(8, 5 + i + i);
                string healthString = characters[i].GraphicHealth();

                //Console.WriteLine($"{characters[i]}: {healthString}");


                Console.WriteLine($"{(Program.characterTurn == i ? ">" : " ")} {characters[i]}: {healthString}");
        ...
        public static void EnemyOutput(Character[] characters)
        {
            ...
            for (int i = 0; i < characters.Length; i++)
            {
                ...
                Console.WriteLine($"{(Program.characterTurn == 3 ? ">" : " ")} {characters[i]}: {healthString}");
            }
